...
const getAuthHeader = function getAuthHeader(apiKey, apiSecret, time, nonce) {
    ...
    const hmac = cryptoJs.default.algo.HMAC.create(
        cryptoJs.default.algo.SHA256,
        apiSecret
    )
    ...
    if (request.query)
        hmac.update(
            typeof request.query == 'object'
                ? qs.default.stringify(request.query)
                : request.query
        )
...
class NicehHashApi {
    ...
    apiCall(method, path) {
        ...
        if (pathQuery)
            query = _objectSpread({}, qs.default.parse(pathQuery), {}, query)
